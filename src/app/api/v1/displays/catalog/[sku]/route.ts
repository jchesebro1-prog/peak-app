...
export async function GET(req: Request, { params }: { params: Promise<{ sku: string }> }) {
  ...
  return NextResponse.json(apiEnvelope(publicCatalogPart(part), { readOnly: true }), { headers: { "cache-control": "private, max-age=60", "x-api-read-only": "true", ...displaysRateHeaders(rate) } });
}
