...
/** Delegates to the shared `bomFromQuote` (#205 spec builder T4), which the
 *  spec builder's "Spec from this quote"/"Spec from this Grid design" doors
 *  also call — one implementation of the quote → BOM extraction. */
export async function bomFromQuoteAction(quoteId: string): Promise<Result<{ bom: BomRow[] }>> {
  await requireUser();
  const r = await bomFromQuote(quoteId);
  return r.ok ? { ok: true, bom: r.rows } : r;
}
