...
export default function DesignClient({
  ...
  // A design's budget is either a parametric tier estimate or (when
  // budgetSource === "grid") a Grid-BOM list-price sum — two different
  // bases. Summing across all designs is still useful as a rough total, but
  // flag it when the mix occurs rather than implying every dollar in it is a
  // tier estimate. Gated on budgetSource, NOT quoteId: a quoteId can be set
  // with no linked Grid project, in which case the budget is still an
  // ordinary parametric number (review round 2, Finding 1).
  const hasGridLinkedBudget = raw.some((d) => d.budgetSource === "grid");
              ...
              <div style={{ textAlign: "right" }}>
                <div style={{ display: "flex", alignItems: "baseline", gap: 8, justifyContent: "flex-end" }}>
                  ...
                  <span style={{ fontSize: 11, color: "#9aa0ab" }}>
                    {sel.budgetSource === "grid" ? "from linked Grid design" : `est. · ${(sel.tier || "better").replace(/^./, (c) => c.toUpperCase())}`}
                  </span>
                </div>
              ...
              <div key={d.id} style={{ display: "flex", flexDirection: "column", background: "#fff", border: "1px solid #ececf0", borderTop: `3px solid ${ACCENT}`, borderRadius: 12, boxShadow: "0 1px 2px rgba(0,0,0,.04)", overflow: "hidden" }}>
                <div style={{ padding: "15px 16px 0", flex: 1 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginTop: 15 }}>
                    ...
                    <span style={{ fontSize: 11, color: "#9aa0ab" }}>
                      {d.budgetSource === "grid" ? "from linked Grid design" : `est. · ${(d.tier || "better").replace(/^./, (c) => c.toUpperCase())}`}
                    </span>
                  </div>
