 ...
 * (inspection twin of the flame-test QuoteBuilder). Holds the ephemeral
 * builder state (customer, venue toggles, line-set counts, level,
 * margin/rate knobs) and previews pricing live with the inspection-engine
 * math inlined below (the engine module itself pulls in the server
 * doc-store, so it can't be imported into a client bundle — Save/Approve
 * re-price server-side in actions.ts).
 */
...
type PerVenue = { id: string; label: string; lineSets: number; laborCost: number };
type Trip = {
  miles: number;
  minutes: number;
  mileageCost: number;
...
const CSS = `
  select.inq-sel { -webkit-appearance: none; appearance: none; background-image: url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='10' height='6' viewBox='0 0 10 6'><path d='M1 1l4 4 4-4' fill='none' stroke='%238c919c' stroke-width='1.5'/></svg>"); background-repeat: no-repeat; background-position: right 12px center; padding-right: 34px !important; }
  @media (max-width: 900px) {
    .inq-grid { grid-template-columns: 1fr !important; }
    .inq-side { position: static !important; }
  }
`;
...
export function QuoteBuilder({
  ...
  const [level, setLevel] = useState(initial.level === 2 ? 2 : 1);
  ...
  const letterHref = savedId ? "/inspections/letter?id=" + encodeURIComponent(savedId) : "";
  ...
  function pickCustomer(id: string) {
    ...
    const sel: Record<string, { on: boolean; lineSets: string }> = {};
    locs.forEach((l) => {
      sel[l.id] = { on: !!l.primary || locs.length === 1, lineSets: "" };
    });
  ...
  function setLineSets(locId: string, val: string) {
    ...
    setVenueSel((prev) => {
      const cur = prev[locId] || { on: true, lineSets: "" };
      return { ...prev, [locId]: { ...cur, lineSets: clean, on: true } };
    });
    dirty();
  }
  ...
  const selectedVenues: VenueIn[] = locations
    ...
    .map((l) => ({
      id: l.id,
      label: l.label,
      lineSets: +(venueSel[l.id]?.lineSets || 0) || 0,
      coords: l.coords,
  ...
  function buildForm(): FormData {
    ...
    fd.set(
      "venues",
      JSON.stringify(
        selectedVenues.map((v) => ({ id: v.id, label: v.label, lineSets: v.lineSets }))
      )
    );
    return fd;
  }
  function doSave() {
    ...
    startTransition(() => saveInspectionQuote(buildForm()));
  }
  function doApprove() {
    if (!canSave || pending) return;
    startTransition(() => approveInspectionQuote(buildForm()));
  }
  ...
  const lineSetsTotal = r?.lineSetsTotal || 0;
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16, minWidth: 0 }}>
          ...
          >
            <div>
              ...
              <SearchableSelect className="inq-sel" value={customerId} onValueChange={pickCustomer}
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Lakefront PAC — Annual rigging inspection"
                style={FIELD}
              />
            </div>
            <div>
                ...
                {levels.map((lv) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        setLevel(lv.key === 2 ? 2 : 1);
                        dirty();
                      }}
                      style={{
                        fontFamily: "var(--font-ui)",
                        textAlign: "left",
                        border: "1.5px solid " + (on ? accent : "#e4e7ec"),
                        background: on ? "color-mix(in srgb, " + accent + " 7%, #fff)" : "#fff",
                        borderRadius: 10,
                        padding: "11px 13px",
                        cursor: "pointer",
                      }}
                    >
                      <div style={{ fontSize: 13, fontWeight: 700, color: on ? accent : "#16181d" }}>
                        {lv.long}
                      </div>
                      <div style={{ fontSize: 11, color: "#9aa0ab", marginTop: 3, lineHeight: 1.45 }}>
                        {lv.blurb}
                      </div>
                    </button>
                  );
            ...
            <div>
              <label style={LABEL}>
                Scope notes{" "}
                <span style={{ textTransform: "none", letterSpacing: 0, fontWeight: 500, color: "#c0c5cd" }}>
                  · carried onto the inspection brief
                </span>
              </label>
              <textarea
                value={notes}
                ...
                rows={2}
                placeholder="e.g. (32) single-purchase line sets, fire curtain, FOH catwalks…"
          ...
          {hasCustomer ? (
            ...
            >
              ...
              >
                <div style={{ fontSize: 14.5, fontWeight: 600 }}>Venues to inspect</div>
                <div style={{ fontSize: 11.5, color: "#9aa0ab" }}>
                  Toggle a venue on and enter its line-set count
                </div>
              </div>
              <div
                style={{
              ...
              >
                <span />
                <span>Venue</span>
                <span style={{ textAlign: "center" }}>Line sets</span>
                <span style={{ textAlign: "right" }}>Inspection</span>
              </div>
              {locations.map((l) => {
                const st = venueSel[l.id] || { on: false, lineSets: "" };
                const on = !!st.on;
                ...
                return (
                  ...
                  >
                    ...
                    <input
                      ...
                      min={0}
                      value={st.lineSets}
                      onChange={(e) => setLineSets(l.id, e.target.value)}
                      disabled={!on}
                      placeholder="0"
                      style={{
                        width: "100%",
              ...
              >
                Travel is charged once for the whole trip and shared across venues; each line set
                takes {baseRates.lineSetMinutes} min plus {baseRates.baseHours} base hours per
                visit{level === 2 ? ", ×" + baseRates.level2Mult + " for the Level 2 in-depth pass" : ""}.
                A {money(baseRates.minFee)} minimum applies to the whole quote.
              </div>
            </div>
          ) : (
            <div
            ...
            >
              Pick a customer to load its venues and auto-price the inspection.
            </div>
          )}
        </div>

        {/* ===== BREAKDOWN SIDEBAR ===== */}
        <div className="inq-side" style={{ position: "sticky", top: 16 }}>
          <div
            style={{
              background: "#fff",
              border: "1px solid #ececf0",
          ...
          >
            ...
            <div style={{ padding: "15px 18px" }}>
              ...
              <SubHead style={{ marginTop: 16 }}>
                Inspection · {r ? r.inspectHours + " hrs" : "—"}
                {level === 2 && r ? " · L2 ×" + r.levelMult : ""}
              </SubHead>
              <BreakRow
                label={"Base visit · " + baseRates.baseHours + " hrs setup & write-up"}
                value={money(r?.baseCost || 0)}
              />
              {(r?.perVenue || []).map((p) => (
                <BreakRow
                  key={p.id}
                  label={p.label + " · " + p.lineSets + " line set" + (p.lineSets === 1 ? "" : "s")}
              ...
              <div style={{ borderTop: "1px solid #eceef1", marginTop: 13, paddingTop: 12 }}>
                ...
                {r?.minApplied && (
                  <div style={{ fontSize: 10.5, color: "#b4543a", margin: "-2px 0 8px", lineHeight: 1.45 }}>
                    Raised to the {money(baseRates.minFee)} minimum — labor + travel came to{" "}
                    {money(r.sellRaw)} with margin.
                  </div>
                )}
                <div
                  style={{
              ...
              {ratesOpen && (
                ...
                >
                  ...
                  <div style={{ fontSize: 10.5, color: "#9aa0ab", lineHeight: 1.5 }}>
                    {baseRates.lineSetMinutes} min/line set · {baseRates.baseHours} base hrs per
                    visit · Level 2 ×{baseRates.level2Mult} · travel time rounded up to{" "}
                    {baseRates.travelRoundMin} min · {money(baseRates.minFee)} job minimum ·{" "}
              ...
              {isApproved && (
                ...
                >
                  ...
                  <div style={{ fontSize: 11.5, color: "#3a6650", marginTop: 6, lineHeight: 1.5 }}>
                    Accepted — now a requested inspection, ready to schedule.
                  </div>
                  <Link
