 ...
 *
 * Moved verbatim from scripts/daylite-ids.ts to src/lib/daylite/ids.ts on
 * 2026-09-24 (D180 follow-up, Task 10) so modules under src/ — which Next
 * bundles — can import it without reaching into scripts/. norm() and hash()
 * are untouched; scripts/daylite-ids.ts now re-exports from here.
 */
...
/** A repair job spawned from a Daylite service-pipeline history row. */
export const repairId = (name: string, company: string): string =>
  "RP-dl-" + hash(norm(name) + "|" + norm(company));

/** A system quote spawned from an open Daylite opportunity. */
export const quoteId = (name: string, company: string): string =>
  "Q-dl-" + hash(norm(name) + "|" + norm(company));
